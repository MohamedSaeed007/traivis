@if (
    ($business_id != auth()->user()->id && ($userType == 'user' && request()->segment(1) == 'p')) ||
        ($userType != 'user' && request()->segment(1) == 'b' && $business && $business->created_by != auth()->id() && $showButton))
    <button type="button"
        class="group flex justify-center items-center rounded-full border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-traivis-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-traivis-500  ">
        ...
        <a
            href="{{ route('chat-conversations', ['id' => request()->segment(2), 'type' => request()->segment(1)]) }}">Message</a>
