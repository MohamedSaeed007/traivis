...
<button @click="open= !open" type="button"
    ...
    @endisset
    {{-- @include('homes.social.layouts.partials.svg-icons.'.$icon, ['svgClass'=>'mr-3 flex-shrink-0 h-6 w-6
    text-gray-400 group-hover:text-gray-500'])--}}

    <span class="flex-1"> {{$text}} </span>
    <!-- Expanded: "text-gray-400 rotate-90", Collapsed: "text-gray-300" -->

    <x-svg-icons.fi-rr-angle-right ::class="{'text-gray-400 rotate-90': open, 'text-gray-300': !open }"
        class="ml-3 flex-shrink-0 h-4 w-4 transform group-hover:text-gray-400 transition-all ease-in-out duration-150" />
