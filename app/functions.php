...
if (!function_exists('view_template')) {
    function view_template($view = null, $data = [], $mergeData = [])
    {
        ...
        $html = view()
            ->make($view_dir . 'dashboard.layouts.components.header', $data, $mergeData)
            ->render();
        $html .= view()
            ->make($view_dir . $view, $data, $mergeData)
            ->render();
        $html .= view()
            ->make($view_dir . 'dashboard.layouts.components.footer', $data, $mergeData)
            ->render();
...
if (!function_exists('view_template_part')) {
    ...
    {
        return view()
            ->make(theme($view), $data, $mergeData)
            ->render();
    }
}
        ...
        if ($media) {
            ...
            if ($media) {
                ...
                foreach ($sizes as $img_size => $name) {
                    ...
                    if ($source == 'public') {
                        if (
                            file_exists(public_path("uploads/images/{$thumb_size}" . $media->slug_ext))
                        ) {
                            $url_path = asset("uploads/images/{$thumb_size}" . $media->slug_ext);
                        } else {
                            $url_path = asset('uploads/placeholder-image.png');
                        }
                    } elseif ($source == 's3') {
                        try {
                            $url_path = \Illuminate\Support\Facades\Storage::disk('s3')->url(
                                "uploads/images/{$thumb_size}" . $media->slug_ext
                            );
    ...
    switch ($metadata['fileformat']) {
        ...
        case 'mp4':
            if (isset($metadata['quicktime']['moov']['subatoms']['0']['creation_time_unix'])) {
                $creation_date =
                    (int) $metadata['quicktime']['moov']['subatoms']['0']['creation_time_unix'];
            }
            break;
    }
 ...
 */
// Update By Mondy
function do_enroll($user_id, $course_id, $transaction_id = 0)
{
    $course = Course::find($course_id);
    $maker = $course->maker;
    $data = [
        'course_price' => $course->sale_price ?? $course->price,
        'transaction_id' => $transaction_id,
        'status' => 'success',
    ];

    $user = User::find($user_id);
    $user->courses()->attach($course_id, $data);
    //Increment Achievement Progress
    $maker->addProgress(new GotStudents(), 1);

    $acceptedOffer = null;
    if (!empty($user->influencer_offer_id)) {
        $acceptedOffer = InfluencerOffer::where('id', $user->influencer_offer_id)
            ->with('campaign')
    ...
    if (!empty($acceptedOffer)) {
        if (!empty($data['course_price'])) {
            $earnings = $data['course_price'] * $acceptedOffer->campaign->commission / 100;
            $acceptedOffer->enrolled += 1;
            $acceptedOffer->earnings += $earnings;
            $acceptedOffer->save();
        }
        $user->influencer_offer_id = null;
        $user->save();
    } else {
        $dateYearAgo = Carbon::now()->subYear();
        if ($user->created_at > $dateYearAgo && !empty($user->user_karma_share_id)) {
            $karmaShare = UserKarmaShare::find($user->user_karma_share_id);
            $karmaShare->enrolled += 1;
            if (!empty($data['course_price'])) {
                $earnings = $data['course_price'] * config('options.karma_commission') / 100;
                $karmaShare->earnings += $earnings;

                \App\Models\KarmaEarning::create([
                    'karma_user_id' => $karmaShare->user_id,
...
if (!function_exists('complete_content')) {
    function complete_content($content, $user)
    {
        ...
        $is_completed = Complete::whereContentId($content->id)
            ->whereUserId($user->id)
            ->first();
        ...
        $completes = Complete::whereUserId($user->id)
...
if (!function_exists('star_rating_field')) {
    function star_rating_field($current_rating = 0.0, $echo = false)
    {
...
if (!function_exists('star_rating_generator')) {
    function star_rating_generator($current_rating = 0.0)
    {
...
if (!function_exists('has_review')) {
    ...
    {
        return Review::whereUserId($user_id)
            ->whereCourseId($course_id)
            ->first();
    }
}
            ...
            'bank_transfer' => [
                ...
                'admin_form_fields' => [
                    'notes' => [
                        ...
                        'label' => 'Notes',
                        'desc' =>
                            'Write notes for the instructor about the bank payment. e.g. <br /> <code>Please note that it takes approximately 2 to 7 days to process your withdraw via bank transfer. Sometimes it may take longer. If you do not receive withdrawal after 7 days, please contact our customer support.</code>'
                    ]
                ],
                ...
                case 'https':
                    $value = preg_replace_callback(
                        ...
                        function ($match) use ($protocol, &$links, $attr) {
                            ...
                            return '<' .
                                array_push($links, "<a $attr href=\"$protocol://$link\">$link</a>") .
                                '>';
                        },
                        $value
                    );
...
if (!function_exists('cookie_message_html')) {
    function cookie_message_html()
    {
        ...
        $link =
            "<a href='" .
            route('post_proxy', get_option('privacy_policy_page')) .
            "'>" .
            __t('read_privacy_policy') .
            '</a>';
        $msg = str_replace('{privacy_policy_url}', $link, $msg);

        return '<div class="cookie_notice_popup">
        <div class="cookie_notice_msg">' .
            $msg .
            '</div>
