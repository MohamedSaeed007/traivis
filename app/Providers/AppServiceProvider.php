    ...
    public function boot()
    {
        ...
        if (file_exists(base_path('.env'))) {
            try {
                 ...
                 */
                $configs['options']['allowed_file_types_arr'] = array_filter(
                    explode(',', array_get($options, 'allowed_file_types')),
                );
                /**
                ...
                require_once get_theme()->path . 'functions.php';
            } catch (Exception $e) {
                //
            }
        } else {
            if (!strpos(request()->getPathInfo(), 'installations')) {
                exit("<script>location.href= '" . url('installations') . "' </script>");
            }
        }
    }
}
