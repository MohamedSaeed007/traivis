...
class LoginController extends Controller
{
    ...
    public function confirm($userId, $confirmationCode)
    {
        ...
        $user = User::where(['id' => $userId, 'activation_token' => $confirmationCode])->first();
        if (!$user) {
            $q = User::where(['id' => $userId])->first();
            if ($q && $q->active == 1) {
    ...
    public function verifyOtp(Request $request, $email)
    {
        ...
        $user = User::where(['email' => $newemail, 'otp' => $otp])->first();
        if ($user) {
            $user->otp_status = 1;
            $user->otp = null;
            $user->save();
    ...
    public function reset_password($token = '')
    {
        ...
        if ($user->count() >= 1) {
            return view('auth.reset')->with(['verCode' => $token]);
        } else {
