...
use App\Models\Individual_Current_Job;
use App\Models\Individual_Language;
use App\Models\Individual_Preferred_Job;
use App\Models\IndividualIndustry;
use App\Models\IndividualOccupation;
use App\Models\IndividualSkillWanted;
use App\Models\IndividualTopSkill;
...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $getNotifications = UserNotification::where([
            ['notifiable_user_id', Auth::user()->id], ['is_read', 0]
        ])->limit(5)->latest()->get();
    ...
    public function profileSettings()
    {
        ...
        if (Auth::check() && Auth::user()->user_type == 'student') {
            $individualLanguages = Individual_Language::where('user_id',
                Auth::user()->id)->pluck('language_id')->toArray();

            $individualSkillsWanted = IndividualSkillWanted::where('user_id',
                Auth::user()->id)->pluck('skill_id')->toArray();

            $individualTopSkills = IndividualTopSkill::where('user_id',
                Auth::user()->id)->pluck('skill_id')->toArray();

            $individualIndustries = IndividualIndustry::where('user_id',
                Auth::user()->id)->pluck('industry_id')->toArray();

            $individualOccupation = IndividualOccupation::where('user_id',
                Auth::user()->id)->pluck('occupation_id')->toArray();

            $highest_degrees = HighestDegree::get();
            $occupations = Occupation::get();
            $industries = Industry::get();
            $skills = Skill::get();
            $indUser = Individual::where('user_id', Auth::user()->id)->first();
            $currentJob = Individual_Current_Job::where('user_id', Auth::user()->id)->first();
            $major_fields = Major::get();

            return view(theme('dashboard.settings.profile'),
                compact('title', 'individualLanguages', 'currentJob', 'individualOccupation', 'individualIndustries',
                    'industries', 'individualSkillsWanted', 'individualTopSkills', 'highest_degrees', 'skills',
                    'occupations', 'indUser', 'major_fields', 'title'));
        }
        if (Auth::check() && (Auth::user()->user_type == 'admin' || Auth::user()->user_type == 'instructor')) {
            $occupations = Occupation::get();
            $industries = Industry::get();
            $tcUser = TrainingCenter::where('user_id', Auth::user()->id)->first();
            $tcLanguages = TC_Language::where('user_id', Auth::user()->id)->pluck('language_id')->toArray();

            return view(theme('dashboard.settings.tc_profile'),
                compact('occupations', 'industries', 'tcUser', 'tcLanguages', 'title'));
        }

        if (Auth::check() && Auth::user()->user_type == 'trainer') {
            $occupations = Occupation::get();
            $industries = Industry::get();
            $tcUser = Trainer::where('user_id', Auth::user()->id)->first();
            $tcLanguages = TR_Language::where('user_id', Auth::user()->id)->pluck('language_id')->toArray();

            return view(theme('dashboard.settings.tr_profile'),
                compact('occupations', 'industries', 'tcUser', 'tcLanguages', 'title'));
        }
    }
    ...
    public function profileSettingsPost(Request $request)
    {
        ...
        $chkLangs = IndividualIndustry::where('user_id', $user->id)->get();

        if (count($chkLangs) > 0) {
            $lObj = IndividualIndustry::where('user_id', $user->id)->delete();
        }

        if ($request->input('preferred_industry')) {
            foreach ($request->input('preferred_industry') as $indus) {
                $lObj = new IndividualIndustry();
                $lObj->user_id = $user_id;
        ...
        $chkLangs = IndividualOccupation::where('user_id', $user->id)->get();

        if (count($chkLangs) > 0) {
            $lObj = IndividualOccupation::where('user_id', $user->id)->delete();
        }

        if ($request->input('prefered_study_language')) {
            foreach ($request->input('prefered_study_language') as $occ) {
                $lObj = new IndividualOccupation();
                $lObj->user_id = $user_id;
