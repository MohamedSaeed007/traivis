...
class CertificateController extends Controller
{
    ...
    public function uploadCertificateCreate(Request $request)
    {
        ...
        $checkUserAdminIsCompany = CompanyWorkforce::join('users', 'users.id', 'company_workforce.company_id')
    ...
    public function certificateView($certificate)
    {
        dd($certificate);
        return view('pages.student-certificate-view');
    }

    public function getCertifiedCourse()
    {
        $data = CompanyWorkforce::join('certificate_uploads', 'certificate_uploads.user_id',
            'company_workforce.user_id')
    ...
    public function getResult()
    {
        $data = Attempt::join('company_workforce', 'company_workforce.user_id', 'attempts.user_id')
            ->where([
                ['company_workforce.company_id', Auth::user()->id],
                ['attempts.status', 'finished'], ['attempts.is_reviewed', 1],
            ])->get();
