...
use App\Models\RequiredCourseDepartment;
use App\Models\RequiredCoursePosition;
use App\Models\RequiredCourseProject;
...
class TrainingMatrixController extends Controller
{
    ...
    public function getMatrixStructureFn(Request $request)
    {
        ...
        return Datatables::of($data)
            ...
            //   foreach ($projects as $project) {
            //     $projectId .= isset($project->company_project['id']) ? $project->company_project['id'] :  "". "</br>";
            //   }
            // }
            //   return  $projectId;
            // })
    ...
    public function deleteMatrixStructure(Request $request)
    {
        ...
        $checkInRequiredCourseWorkForce = RequiredCourseWorkforce::where('course_id', $courseId)->get();
        if ($checkInRequiredCourseWorkForce) {
            foreach ($checkInRequiredCourseWorkForce as $courseWorkForce) {
                $courseWorkForce->delete();
            }
            // return response()->json(['status' => 'false','message'=>"The course already asigned in training matrix"], 200);
        }
        $obj = CompanyRequiredCourse::where('id', $request->input('course_id'))->first();
        $obj->delete();
    ...
    public function trainingMatrix(Request $request)
    {
        ...
        return view('company.training-matrix')->with([
    ...
    public function getMatrixData(Request $request)
    {
        ...
        if ($request->department != '') {
            $data->whereIn('cd.department_id', $request->department);
        }

        if ($request->position != '') {
            $data->whereIn('cp.position_id', $request->position);
        }

        if ($request->project != '') {
            $data->whereIn('cj.project_id', $request->project);
        }
        ...
        $table = Datatables::of($data)
            ...
            ->editColumn('department', function ($model) {
                ...
                if ($departments) {
                    foreach ($departments as $department) {
                        if (isset($department->workforce_department_data['en_department'])) {
                            $departmentId .= $department->workforce_department_data['id'];
                ...
                return '<a href="/company/departments/'.$departmentId.'">'.$department_names.'</a>';
            })
            ->editColumn('position', function ($model) {
                ...
                if ($positions) {
                    foreach ($positions as $position) {
                        if (isset($position->workforce_positions_data['position_en'])) {
                            $positionId .= $position->workforce_positions_data['id'];
                            $position_names .= $position->workforce_positions_data['position_en'].'</br>';
                        }
                    }
                }

                return '<a href="/company/positions/'.$positionId.'">'.$position_names.'</a>';
            })
            // ->editColumn('project', function ($model) {
            ...
            //       {
            //     $projectId .= $project->workforce_projects_data['id'] . "</br>";
            //       }
            //   }
            // }
            //   return  $projectId;
            ...
            ->editColumn('project', function ($model) {
                ...
                if ($projects) {
                    foreach ($projects as $project) {
                        if (isset($project->workforce_projects_data['en_project'])) {
                            $projectId .= $project->workforce_projects_data['id'];
                            $project_names .= $project->workforce_projects_data['en_project'].'</br>';
                        }
                    }
                }

                return '<a href="/company/projects/'.$projectId.'">'.$project_names.'</a>';
            });
        ...
        foreach ($courselist as $course) {
            ...
            $table->editColumn('nullcol_'.$i, function ($model) use ($course, $company_id, $type, $userType) {
                $today = Carbon::now();
                ...
                $assignedCourse = CompanyWorkforce::select(
                    ...
                    ->where(function ($q) use ($type, $company_id, $userType) {
                        if ($userType == 'company') {
                            if ($type != null) {
                                $q->where('company_workforce.company_id', $type);
                            } else {
            ...
            data-course-department="'.$departmentId.'"
            data-course-project="'.$projectId.'"
            data-course-position="'.$positionId.'"
            ...
            data-certificate-status="'.$assignedCourse->certificate_status.'"
             onclick="showEditModal(this)">'.$label.'</div>';
                    ...
                    if ($course_departments) {
                        foreach ($course_departments as $course_departmentsItem) {
                            array_push($cdepArr, $course_departmentsItem->department_id);
                        }
                    }
                    // dd($cdepArr);
                    $udepArr = [];
    ...
    public function assignRequiredCourse(Request $request)
    {
        ...
        // dd(isset($request->input('index_id')));
        $departmentId = $request->input('course-department');
        $projectId = $request->input('course-project');
        $positionId = $request->input('course-position');

        // dd($departmentId);
        $company_id = Session::get('company_id');
        $start = $request->input('start_date');
        $index = $request->input('index_id');
        ...
        // dd($obj);
        if ($obj->id) {
            return response()->json(['id' => $obj->id, 'status' => 'success'], 200);
        } else {
            return response()->json(['status' => 'error'], 422);
        }
    }
