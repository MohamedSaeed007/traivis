...
use App\Models\CompanyPurchaseTemporary;
...
class BiddingController extends Controller
{
    ...
    {
        $category = Category::orWhere('step', 1)->get();

        return view('company.bidding', compact('category'));
    }
    ...
    {
        $data = Bidding::join('categories', 'categories.id', 'biddings.category_id')
            ->select(
            ...
            ->where([
                ['biddings.company_id', Auth::user()->id],
        ...
        ";
            ...
            // })
            ->editColumn('view_result', function ($model) {
                $html = "<a href='bidding-list-result/$model->id' class='btn btn-success ' data-id='$model->id'  > View  Result</a> ";
                //  &nbsp; &nbsp;<a class='btn btn-danger' > Reject </a>" ;
                return $html;
            })
            ->rawColumns(['s#', 'course', 'details', 'view_result'])
            ->make(true);
    }

    public function getBiddingListResult(Request $request)
    {
        $bidding = $request->bidding;

        $data = CouponCode::where('bidding_id', $bidding)->get();
        ...
        return DataTables::of($data)
            ...
            ->editColumn('course', function ($model) {
                if ($model->couponCourse) {
                    return $model->couponCourse->title;
                }
        ...
        ";
            ...
            ->editColumn('promocode', function ($model) {
                return $model->coupon_code.'<br>'.'<small>expiry date: </small>'.$model->valid_to;
            })
            ->editColumn('status', function ($model) {
                $html = '';
                if ($model->is_company_aprove == 1) {
                    $html = 'Approved';
                } elseif ($model->is_company_aprove == 2) {
                    $html = 'Rejected';
                } else {
                    $html = "<a href='status-change-company/$model->id?value=1' class='btn btn-success ' data-id='$model->id'  > Approve</a>
        <a href='status-change-company/$model->id?value=2' class='btn btn-danger ' data-id='$model->id'  > Reject</a> ";
                }
                //  &nbsp; &nbsp;<a class='btn btn-danger' > Reject </a>" ;
                return $html;
            })
            ->rawColumns(['s#', 'course', 'details', 'instructor_name', 'document', 'promocode', 'status'])
            ->make(true);
    }
    ...
    public function resultBidding(Request $request, $id)
    {
        $bidding = $id;

        return view('company.bidding-result', compact('bidding'));
    }

    public function statusChangeCompany(Request $request, $id)
    {
        // dd($request->all());
        $value = $request->value;
        $coupon = CouponCode::find($id);
        $coupon->is_company_aprove = $value;
        $coupon->save();
