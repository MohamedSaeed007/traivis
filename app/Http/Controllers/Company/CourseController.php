...
class CourseController extends Controller
{
    ...
    {
        $skill = Skill::select('id', 'en_skill', 'ar_skill')->get();
        $occupation = Occupation::select('id', 'en_occupation', 'ar_occupation')->get();
        $industry = Industry::select('id', 'en_name', 'ar_name')->get();

        return view('company.course-search', compact('skill', 'occupation', 'industry'));
    }
    ...
    public function getPurchasedCourses(Request $request)
    {
        ...
        return Datatables::of($data)
            ...
            ->rawColumns([
                's#', 'course_en', 'validity', 'department', 'position', 'project', 'payment_status', 'actions'
            ])
            ->make(true);
    }
    ...
    public function saveBiddingPrice(Request $request)
    {
        ...
        if (Auth::check()) {
            $bidding = new Bidding();
            $bidding->category_id = $request->get('category');
            ...
            if (!$request->types == 1) {
                ...
                if ($auth->user_type == 'project-manager') {
                    ...
                    return Response::json([
                        ...
                        'message' => 'Please waiting for company admin approval'
                    ]);
                }
                if ($auth->user_type == 'company') {
    ...
    {
        $data = CourseUser::where([['purchased_by', Auth::user()->id], ['status', 'success']])->orderBy('id',
            'DESC')->get();
