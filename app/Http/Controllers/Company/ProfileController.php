...
class ProfileController extends Controller
{
    ...
    public function getProfileData()
    {
      ...
      END) AS gender'),
        ...
        $occupations = CompanyOccupation::select('occupation.en_occupation as en_occupation',
            'occupation.ar_occupation as ar_occupation')
    ...
    public function getProfile()
    {
        ...
        $userId = Auth::user()->id;
        $checkCompany = CompanyUser::where('user_id', $userId)->first();
        // $country = Country::get();
        // dd($countries);
        if ($checkCompany) {
            $company = Company::find($checkCompany->company_id);
            $companyLanguages = CompanyLanguage::where('company_id',
                $company->id)->pluck('language_id')->toArray();
            $companyIndustries = CompanyIndustry::where('company_id',
                $company->id)->pluck('industry_id')->toArray();
            $companyOccupations = CompanyOccupation::where('company_id',
                $company->id)->pluck('occupation_id')->toArray();
    ...
    public function createProfile(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($user) {
                ...
                $company->status = 1;
                $company->save();
                $contact_details = new CompanyContactDetail();
                ...
                if ($request->get('occupation')) {
                    foreach ($request->get('occupation') as $occ) {
                        $occupation = new CompanyOccupation();
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($company) {
            ...
            $userInfo = CompanyUser::where('company_id', $company->id)->first();

            if ($userInfo) {
                $getUser = User::find($userInfo->user_id);
            }
            if ($request->get('industry')) {
                $chkInd = CompanyIndustry::where('company_id', $company->id)->get();
                if (count($chkInd) > 0) {
                    CompanyIndustry::where('company_id', $company->id)->delete();
                }
                foreach ($request->get('industry') as $ind) {
                    $industry = new CompanyIndustry();
                    $industry->company_id = $company->id;
            ...
            if ($request->get('occupation')) {
                $chkOcc = CompanyOccupation::where('company_id', $company->id)->get();
                if (count($chkOcc) > 0) {
            ...
            if ($request->get('pre_language')) {
                $chkLan = CompanyLanguage::where('company_id', $company->id)->get();
                if (count($chkLan) > 0) {
                    CompanyLanguage::where('company_id', $company->id)->delete();
                }
                foreach ($request->get('pre_language') as $prelan) {
                    $lan = new CompanyLanguage();
                    $lan->company_id = $company->id;
