...
use App\Models\WorkforceProject;
...
class SettingsController extends Controller
{
    ...
    public function getPositionsFn(Request $request)
    {
        ...
        $data = CompanyPosition::select('id', 'company_id', 'position_en', 'position_ar')
            ->where(function ($q) use ($userType, $company_id) {
                ...
                if ($userType == 'contractor' || $userType == 'project-manager') {
    ...
    public function deletePosition(Request $request)
    {
        ...
        $checkInRequiredCourseWorkForceUser = RequiredCourseWorkforce::where('user_id', $userId)->get();

        if ($checkInRequiredCourseWorkForceUser) {
            foreach ($checkInRequiredCourseWorkForceUser as $courseWorkForceUser) {
                $courseWorkForceUser->delete();
            }
            // return response()->json(['status' => 'false','message'=>"The course already asigned in training matrix"], 200);
        }
        if ($userId) {
            $userEmailDelete = User::find($userId);
            $userEmailDelete->email = $userEmailDelete->email.'-deleted';
            $userEmailDelete->save();
            $userEmailDelete->delete();
