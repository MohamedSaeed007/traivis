...
use App\Models\CompanyIndividualRequest;
...
class WorkForceController extends Controller
{
    ...
    public function getWorkforceData(Request $request)
    {
        ...
        return Datatables::of($data)
            ...
            ->rawColumns([
                's#', 'fname', 'added_by', 'work_id', 'email', 'department', 'position', 'project', 'actions'
            ])
            ->make(true);
    }
    ...
    public function searchIndidual(Request $request)
    {
        ...
        if ($user && isset($checkAlreadyCompanyUser->company_id) == false) {
            $companyRequest = CompanyIndividualRequest::where([
                ['company_user_id', Auth::user()->id], ['individual_user_id', $user->id]
            ])->first();
