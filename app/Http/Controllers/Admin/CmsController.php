...
class CmsController extends Controller
{
    ...
    public function do_create(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if ($pages->id) {
                return redirect()->back()->with(['message' => 'Pages Successfully Added']);
            } else {
    ...
    public function view($page_id)
    {
        ...
        return view('admin.pages.page_view')->with(['data' => $data, 'user' => $user]);
    }
...
//        Code for editing the details of the page
            ...
            if ($validator->passes()) {
                ...
                if ($pageSection) {
                    if ($request->file('profile_file')) {
                        foreach ($request->file('profile_file') as $key => $img) {
                            $pageSectionImg = PageSectionImage::where('page_id', $page->id)->where('section_id',
                                $key + 1)->first();
                            if ($pageSectionImg) {
                ...
                if ($page->id) {
                    return redirect()->back()->with(['message' => 'Pages Successfully Updated']);
                }
            } else {
    ...
    public function show($slug)
    {
        ...
        if ($data->count() > 0) {
            return view('page_display')->with(['data' => $data, 'user' => $user]);
        } else {
    ...
    public function homeSectionView($page_id)
    {
        ...
        return view('admin.pages.home_section_page_view')->with(['data' => $data, 'user' => $user]);
    }
