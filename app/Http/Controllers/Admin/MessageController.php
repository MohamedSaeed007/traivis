...
class MessageController extends Controller
{
    public function index()
    {
        ...
        if ($auth->user_type == 'admin') {
            ...
            $parentCompany = User::leftjoin('company_workforce', 'company_workforce.company_id', 'users.id')
            ...
            $student = Course::join('course_users', 'course_users.course_id', 'courses.id')
                ->where([['course_users.user_id', Auth::user()->id], ['course_users.status', 'success']])
                ->select('course_users.*', 'courses.user_id as user_id')->groupBy('courses.user_id')->get();
        } elseif ($auth->user_type == 'instructor') {
            $users = User::leftjoin('instructor_trainers', 'instructor_trainers.trainer_user_id', 'users.id')
                ->select(
                    'users.name as name',
                    'users.id as id',
                    'users.user_type as user_type',
                ...
                ->get();
            $student = CourseUser::join('courses', 'courses.id', 'course_users.course_id')
                ->where([['courses.user_id', Auth::user()->id], ['course_users.status', 'success']])
                ->select('course_users.*')->get();
        } elseif ($auth->user_type == 'trainer') {
            $users = User::leftjoin('instructor_trainers', 'instructor_trainers.instructor_user_id', 'users.id')
                ->select(
                    'users.name as name',
                    'users.id as id',
                    'users.user_type as user_type',
        ...
        if ($auth->user_type == 'admin') {
            return view('admin.chats.index')->with([
                'users' => $users, 'completeChats' => $completeChats, 'today' => $today, 'title' => $title
            ]);
        } elseif ($auth->user_type == 'company') {
            return view('company.chats.index')->with([
                'users'     => $users, 'completeChats' => $completeChats, 'today' => $today, 'title' => $title,
                'contactor' => $contactor
            ]);
        } elseif ($auth->user_type == 'student') {
            return view(theme('dashboard.chats.index'))->with([
                'users'         => $users, 'completeChats' => $completeChats, 'today' => $today, 'title' => $title,
                'parentCompany' => $parentCompany, 'student' => $student
            ]);
        } elseif ($auth->user_type == 'instructor' || $auth->user_type == 'trainer') {
            return view(theme('dashboard.instuctor-chat.index'))->with([
                'student' => $student, 'users' => $users, 'completeChats' => $completeChats, 'today' => $today,
                'title'   => $title
            ]);
        }
    ...
    public function getMessages(Request $request)
    {
        ...
        if ($userId) {
            ...
            return response()->json([
                'success'       => true, 'returnHTML' => $returnHTML, 'receiver_id' => $userId, 'title' => $title,
    ...
    public function searchUser(Request $request)
    {
        ...
        if ($auth->user_type == 'admin') {
            ...
            $contactorSearch = CompanyWorkforce::join('users', 'users.id', 'company_workforce.user_id')
                ...
                ->orderBy('users.id', 'desc')
                ->where([
                    ['contractorUser.parent_company', $auth->id],
                    ['users.name', 'like', '%'.$keyword.'%'],
                ])
                ->orderBy('chats.id', 'desc')->groupBy('users.id')->get();
        } elseif ($auth->user_type == 'instructor' || $auth->user_type == 'trainer') {
            if ($auth->user_type == 'instructor') {
                $users = User::leftjoin('instructor_trainers', 'instructor_trainers.trainer_user_id', 'users.id')
                    ...
                    ->where([
                        ['instructor_trainers.instructor_user_id', $auth->id],
                        ['users.name', 'like', '%'.$keyword.'%'],
                    ])
                    ->orWhere('users.user_type', 'admin')
                    // ->where('users.user_type','trainer')
            ...
            if ($auth->user_type == 'trainer') {
                $users = User::leftjoin('instructor_trainers', 'instructor_trainers.instructor_user_id', 'users.id')
                    ...
                    ->where([
                        ['instructor_trainers.trainer_user_id', $auth->id],
                        ['users.name', 'like', '%'.$keyword.'%'],
                    ])
                    ->orWhere('users.user_type', 'admin')
                    // ->where('users.user_type','trainer')
            ...
            $student = CourseUser::join('courses', 'courses.id', 'course_users.course_id')
                ->where([['courses.user_id', Auth::user()->id], ['course_users.status', 'success']])
                ->select('course_users.*')->get();
            $returnHTML = view(theme('dashboard.instuctor-chat.search-result'),
                ['contactorSearch' => $student, 'userSearch' => $users])->render();
        ...
        $returnHTML = view('admin.chats.search-result',
            ['userSearch' => $userSearch, 'contactorSearch' => $contactorSearch])->render();
