...
class CourseController extends Controller
{
    ...
    public function view($slug)
    {
        ...
        $course = Course::whereSlug($slug)
            ->with(['sections', 'sections.contents', 'sections.contents.attachments', 'lectures'])
            ->first();
        if (!$course) {
            abort(404);
        }
        ...
        if (request()->filled('karma')) {
            ...
            // if (!session()->has('user_karma_share_id')) {
                ...
                if (!empty($karmaUser)) {
                    ...
                    $karmaLink = UserKarmaShare::firstOrNew([
                        'course_id' => $course->id,
                        'user_id' => $karmaUser->id
                    ]);
                    if (!empty($acceptedOffer)) {
                        $acceptedOffer->reached += 1;
                        $acceptedOffer->save();
                        if (Auth::check()) {
                            $user = Auth::user();
                            $user->influencer_offer_id = $acceptedOffer->id;
                            $user->save();
                        } else {
                            session()->put('InfluencerOfferID', $acceptedOffer->id);
                        }
                    }
                    $karmaLink->reached += 1;
                    $karmaLink->save();
                    session()->put('user_karma_share_id', $karmaLink->id);
                }
            // }
        }
        ...
        if (Auth::check()) {
            ...
            if ($course->maker->owner->id != auth()->id()) {
                ViewsRepository::createCourseView([$course->id]);
            }
        }

        //TODO: livewire details page

        //        SEOMeta::setKeywords(['key1', 'key2', 'key3', 'key4']);

        $this->seo()
            ->setTitle($title)
            ->setDescription($course->meta_description);
        //            ->setDescription($course->description);
        //        ->($course->description);

        return view('homes.social.layouts.course-details', compact('course', 'title', 'isEnrolled'));
    ...
    public function assignmentView($slug, $assignment_id)
    {
        ...
        return view(
            theme('assignment'),
            compact('course', 'title', 'isEnrolled', 'assignment', 'has_submission'),
        );
    }
    ...
    public function create()
    {
        ...
        $trainer = User::join('instructor_trainers', 'instructor_trainers.trainer_user_id', 'users.id')
            ->where([
                ['instructor_trainers.instructor_user_id', Auth::user()->id],
                ['instructor_trainers.request_status', 1],
            ])
        ...
        return view(
            theme('dashboard.courses.create_course'),
            compact(
                'title',
                'categories',
                'industries',
    ...
    public function store(Request $request)
    {
        ...
        if ($course) {
            ...
            if ($request->occupation) {
                if (count($request->occupation) > 0) {
                    foreach ($request->occupation as $occupation) {
                        $occupations = new CourseOccupation();
            ...
            if (
                (isset($request->trainer) && Auth::user()->user_type == 'instructor') ||
                (isset($request->trainer) && Auth::user()->user_type == 'admin')
            ) {
                //  dd($request->trainer);
    ...
    public function information($course_id)
    {
        ...
        $occupations = Occupation::get();
        $skills = Skill::get();
        $selSkills = CourseSkill::where('course_id', $course->id)
            ->pluck('skill_id')
            ->toArray();
        $selOccupations = CourseOccupation::where('course_id', $course->id)
            ->pluck('occupation_id')
            ->toArray();
        $selIndustries = CourseIndustry::where('course_id', $course->id)
            ->pluck('industry_id')
            ->toArray();
        ...
        return view(
            theme('dashboard.courses.information'),
            compact(
                'title',
                'course',
                'categories',
    ...
    public function informationPost(Request $request, $course_id)
    {
        ...
        if (isset($request->skills) && count($request->skills) > 0) {
            if ($request->get('skills')) {
                $chkInd = CourseSkill::where('course_id', $course->id)->get();
                if (count($chkInd) > 0) {
        ...
        if (isset($request->occupation) && count($request->occupation) > 0) {
            if ($request->get('occupation')) {
                $chkOcc = CourseOccupation::where('course_id', $course->id)->get();
                if (count($chkOcc) > 0) {
                    CourseOccupation::where('course_id', $course->id)->delete();
                }
                foreach ($request->occupation as $occupation) {
                    $occupations = new CourseOccupation();
                    $occupations->course_id = $course->id;
        ...
        if (isset($request->industry) && count($request->industry) > 0) {
            if ($request->get('industry')) {
                $chkInd = CourseIndustry::where('course_id', $course->id)->get();
                if (count($chkInd) > 0) {
    ...
    public function updateLecture(Request $request, $course_id, $item_id)
    {
        ...
        if (is_array($attachments) && count($attachments)) {
            foreach ($attachments as $media_id) {
                $hash = strtolower(str_random(13) . substr(time(), 4) . str_random(13));
                Attachment::create([
                    'belongs_course_id' => $item->course_id,
                    'content_id' => $item->id,
                    'user_id' => $user_id,
                    'media_id' => $media_id,
                    'hash_id' => $hash,
    ...
    public function editItem(Request $request)
    {
        ...
        if ($item->item_type === 'lecture') {
            $form_html = view_template_part(
                'dashboard.courses.edit_lecture_form',
                compact('item', 'vimeoTitle'),
            );
        } elseif ($item->item_type === 'quiz') {
            $form_html = view_template_part(
                'dashboard.courses.quiz.edit_quiz',
                compact('item', 'vimeoTitle'),
            );
        } elseif ($item->item_type === 'assignment') {
            $form_html = view_template_part(
                'dashboard.courses.edit_assignment_form',
                compact('item', 'vimeoTitle'),
            );
        }
    ...
    public function freeEnroll(Request $request)
    {
        ...
        if (!$isEnrolled) {
            ...
            $coursePostUser = User::find($course->user_id);
    ...
    public function contentComplete($content_id)
    {
        ...
        return redirect(
            route('single_' . $go_content->item_type, [$go_content->course->slug, $go_content->id]),
        );
    }
    ...
    public function forceDownload($media)
    {
        ...
        if ($source == 'public') {
            ...
            $path = Storage::disk('s3')->url('uploads/' . $slug_ext);
        }
    ...
    public function assignedCourse()
    {
        ...
        $assignedCourse = Course::join(
            'course_assign_trainers',
            'course_assign_trainers.course_id',
            'courses.id',
        )
            ...
            ->select(
                ...
                'course_assign_trainers.id as course_assign_id',
                'courses.user_id as user_id',
                'course_assign_trainers.status as status',
            )
            ->latest('course_assign_trainers.created_at')
            ->get();
        // dd($assignedCourse);
    ...
    public function statusChangesAssignedCourse(Request $request)
    {
        ...
        if ($status == 'accept') {
            ...
            DB::table('course_user')->insert([
                'course_id' => $course->id,
                'user_id' => $assigned->trainer_user_id,
            ]);
            //notification to trainer
            $notifi = new UserNotification();
    ...
    {
        $title = __t('Request');
        $requestTrainer = InstructorTrainer::join(
            'users',
            'users.id',
            'instructor_trainers.trainer_user_id',
        )
            ->select('instructor_trainers.*')
            ->where('instructor_trainers.trainer_user_id', Auth::user()->id)
            ->get();
        // dd($requestTrainer);
