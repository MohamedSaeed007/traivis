...
class FrontController extends Controller
{
    ...
    public function index()
    {
        ...
        $training_centers = TrainingCenter::where('status', 1)->get();

        return view('home')->with([
            'testimonials' => $testimonials, 'courses' => $courses, 'training_centers' => $training_centers
        ]);
    }
    ...
    public function downloadResourses(Request $request)
    {
        ...
        $free_resources = FreeResource::orderBy('id', 'DESC')->where('status', 1);

        $free_resources = FreeResource::leftJoin('free_resources_skills', 'free_resources.id',
            'free_resources_skills.free_resource_id')
        ...
        if ($request->get('keyword')) {
            if (App::getLocale() == 'ar') {
                $free_resources = $free_resources->where('free_resources.title_ar', 'like',
                    '%'.$request->get('keyword').'%');
            } else {
                $free_resources = $free_resources->where('free_resources.title_en', 'like',
                    '%'.$request->get('keyword').'%');
            }
        }
    ...
    public function resourceDetail(Request $request, $id)
    {
        ...
        $freeResources = FreeResource::where('id', '!=', $free_resource->id)
    ...
    public function courseDetail(Request $request, $slug)
    {
        ...
        $previews = CoursePreview::where('course_id', $course->id)->get();
        $userId = \Auth::check() ? \Auth::user()->id : null;
        $rating = Rating::where('user_id', $userId)->where('type', 'course')->where('post_id',
            $course->id)->first();

        if ($course) {
            $relatedCourses = Course::where('status', 1)->where('course_category_id',
                $course->course_category_id)->where('slug', '!=', $slug)->orderBy('id', 'desc')->take(2)->get();
    ...
    public function viewPage(Request $request, $slug)
    {
        ...
        $banner = '';
        $testimonials = '';
        $checkPage = Page::where('slug', $slug)->first();
        if ($checkPage) {
            $testimonials = Testimonial::where('status', 1)->get();
            $banner = Banner::where('page_id', $checkPage->id)->first();
            $package = Package::limit(4)->where('status', 1)->get();
            $function = Functionality::get();

            return view('pages.index')->with([
                'page'  => $checkPage, 'banner' => $banner, 'testimonials' => $testimonials, 'package' => $package,
    ...
    public function setRating(Request $request)
    {
        ...
        if ($userId && $postId && $type) {
            $chkRate = Rating::where('user_id', $userId)->where('type', $type)->where('post_id',
                $postId)->first();
            if ($chkRate) {
    ...
    public function listCourses(Request $request)
    {
        ...
        //dump($request->all());
        $courses = Course::leftJoin('course_skills', 'courses.id', 'course_skills.course_id')
            ->leftJoin('course_languages', 'courses.id', 'course_languages.course_id')
            ->leftJoin('ratings', 'courses.id', 'ratings.post_id')
            ->orderBy('courses.id', 'DESC')
            ->select('courses.*', DB::raw('avg( ratings.rating ) as avg_p'), 'course_languages.course_id',
                'courses.id as cid')
            ->groupBy('courses.id')
        ...
        } elseif ($request->get('list_type') == 'paid') {
            $courses = $courses->where('courses.cost_per_person', '>', 0);
        }
        ...
        if ($request->ajax()) {
            ...
            return response()->json([
                'success' => true, 'html' => $returnHTML, 'allRatings' => $allRatings, 'count' => $count,
                'courses' => $courses
            ]);
        }
        ...
        return view('pages.courses')->with([
            'courses' => $courses, 'skills' => $skills, 'all_languages' => $all_languages, 'allRatings' => $allRatings,
            'count'   => $count
        ]);
    }
    ...
    public function allCourseRating(Request $request)
    {
        ...
        //  else
        //  {
        $ratings = Rating::where('post_id', $request->get('post_id'))->where('type', 'course')->get();
        //}
    ...
    public function addToWishlist(Request $request)
    {
        ...
        if (!empty($id)) {
            $record = Wishlist::where('course_id', $id)->where('user_id', Auth::user()->id)->first();

            if ($record) {
                $record->delete();

                return response()->json(['success' => true, 'status' => 'remove']);
            } else {
                $wish = new Wishlist();
                $wish->course_id = $id;
                $wish->user_id = $userId;
                ...
                return response()->json(['success' => true, 'status' => 'add']);
            }
        }
    }
