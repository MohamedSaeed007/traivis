...
class AuthController extends Controller
{
    ...
    public function loginPost(Request $request)
    {
        ...
        return redirect()
            ->back()
            ->with('error', __t('login_failed'))
            ->withInput($request->input());
    }

    public function register($userType, Request $request)
    {
        $type = $request->type;
        $userTypes = [
            'company',
    ...
    public function registerPost(Request $request)
    {
        ...
        $messages = [
            'password.regex' =>
                'Your password must be more than 8 characters long, should contain at-least 1 Uppercase, <br> 1 Lowercase, 1 Numeric and <br> 1 special character'
        ];
        ...
        if (in_array($userType, $userTypes)) {
            if ($userType == 'contractor') {
                ...
                if ($auth->user_type == 'company') {
                    ...
                    if ($checkUserSubscription != null && $checkUserSubscription->count > 0) {
                        $companyContractorCount = User::where('parent_company', $auth->id)
                            ->get()
                            ->count();
                        if ($checkUserSubscription != null) {
                            if ($checkUserSubscription->count == $companyContractorCount) {
                                return redirect()
                                    ...
                                    ->with(
                                        'message',
                                        'Your contractor limit is exceed...plese renew package'
                                    );
                            ...
                            return redirect()
                                ...
                                ->with(
                                    'message',
                                    "can't add contractor now ...please subscribe package"
                                );
                            //   return response()->json([
                            //     'status' => false,
            ...
            return back()
                ->with('error', __t('failed_try_again'))
                ->withInput($request->input());
        }
        // dd($request->all());
    }
    ...
    public function sendResetToken(Request $request)
    {
        ...
        Mail::to($user->email)->queue(new UserRegistrationMail('user-password-reset', $user));

        return Redirect::to('forgot-password')->with('message', 'We have sent you an email to reset your password. Please check your email!');
    ...
    public function passwordReset(Request $request, $token)
    {
        ...
        return redirect(route('login'))->with('success', __t('Your Password Changed Successfully'));
    }
    ...
    {
        //        dd(env('TRAIVIS_LINKEDIN_API_KEY'));


        https: //www.linkedin.com/oauth/v2/authorization?client_id=78dpk0ogjwinta&redirect_uri=https%3A%2F%2Ftraivis.test%2Flogin%2Flinkedin%2Fcallback&scope=r_liteprofile+r_emailaddress+w_member_social&response_type=code&state=TwSsYSWhLHai8XHgIZno5Ebe7XIA2WdvZHOJQDr0
