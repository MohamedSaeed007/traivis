...
use App\Models\FreeResourceLanguage;
use App\Models\FreeResourceSkill;
...
class ResourceController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $free_resource = new FreeResource();
    ...
    public function updateForm($id)
    {
        $skills = Skill::get();
        $all_languages = AllLanguage::where('active', 1)->get();
        $free_resource = FreeResource::where('posted_by', Auth::user()->id)->find($id);

        $selLanguages = FreeResourceLanguage::where('free_resource_id',
            $free_resource->id)->pluck('language_id')->toArray();

        $selSkills = FreeResourceSkill::where('free_resource_id',
            $free_resource->id)->pluck('skill_id')->toArray();
        ...
        // ]);
        return view(theme('dashboard.free_resources.edit'),
            compact('skills', 'all_languages', 'free_resource', 'selLanguages', 'selSkills'));
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $chkInd = FreeResourceSkill::where('free_resource_id', $free_resource->id)->get();
        if (count($chkInd) > 0) {
            FreeResourceSkill::where('free_resource_id', $free_resource->id)->delete();
        }
        foreach ($request->get('skills') as $skill) {
            $lan = new FreeResourceSkill();
            $lan->free_resource_id = $free_resource->id;
        ...
        $chkLan = FreeResourceLanguage::where('free_resource_id', $free_resource->id)->get();
        if (count($chkLan) > 0) {
            FreeResourceLanguage::where('free_resource_id', $free_resource->id)->delete();
        }
