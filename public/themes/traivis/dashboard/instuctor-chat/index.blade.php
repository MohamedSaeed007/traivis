                                ...
                                @foreach ($student as $key => $studentPortal)
                                    @if($studentPortal->user)
                                        <div class="chat_list @if ($key==0) active_chat @endif"
                                             data-id="{{ $studentPortal->user->id }}">
                                            <div class="chat_people">
                                                @if($studentPortal->user->avatar )
                                                    <div class="chat_img"><img
                                                                src="{{$studentPortal->user->profile_pic)}}"
                                                                alt="sunil">
                                                    </div>
                                                @else
                                                    <div class="chat_img"><img
                                                ...
                                                <div class="chat_ib">
                                                    @php
                                                        $chatTime = \App\Models\Chat::orWhere('sender_id',$user->id)->orWhere('receiver_id',$studentPortal->user->id)->latest()->first();
                                                    @endphp
                                                    ...
                                                    @if(unreadReciverMessages($studentPortal->user->id))
                                                        <span class='badge badge-warning float-right hide-count{{$studentPortal->user->id}}'> {{unreadReciverMessages($studentPortal->user->id)}} </span>
                                                    @endif
