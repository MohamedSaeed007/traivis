        ...
        <tr>
            ...
            @foreach ($bidding as $key => $biddingRequest)
                <tr>
                    ...
                    <td>
                        @if ($biddingRequest->biddingRequestCategory)
                            {{ $biddingRequest->biddingRequestCategory->category_name }}
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        @if($biddingRequest->couponCodeCheckUser)
                        @if($biddingRequest->couponCodeCheckUser->is_accept == 1 && $biddingRequest->couponCodeCheckUser->accepatnce_status != 1)
                        <button class="btn btn-block btn-primary status-model bidding_id status-change"
                        data-id="{{ $biddingRequest->couponCodeCheckUser->id }}" data-toggle="modal">Confirm</button>
                        @elseif($biddingRequest->couponCodeCheckUser->is_accept == 2)
                                <h3>Rejected</h3>
                        @endif
                        @if ($biddingRequest->couponCodeCheckUser->accepatnce_status == 1 && $biddingRequest->couponCodeCheckUser->is_genarate_coupon == 0)
                        <button class="btn btn-block btn-primary coupon-model coupon_code"
                          data-content="{{$biddingRequest->category_id}}"  data-id="{{ $biddingRequest->couponCodeCheckUser->id }}" data-toggle="modal">Coupen Code</button>
                    @endif
                    @if ($biddingRequest->couponCodeCheckUser->is_genarate_coupon == 1)
                            <p>Coupon Gerated</p>
                            <small>
                               
                            <a target="_blank" href="{{url('dashboard/coupons?type='.$biddingRequest->couponCodeCheckUser->coupon_code)}}">  <span style="color: #ec0625">{{ $biddingRequest->couponCodeCheck->coupon_code }} </span>  </a>
                                
                            </small>
                        @endif
                        <br>
                        @if ($biddingRequest->couponCodeCheckUser->is_company_aprove == 1)
                       <span style="color: green;">Approved </span>
                            @elseif($biddingRequest->couponCodeCheckUser->is_company_aprove ==2)
                        ...
                        <a href="{{url('dashboard/bidding-status-change?value=1',$biddingRequest->id)}}" class="btn btn-block btn-primary  "
                         >Accept</a>

                        <a href="{{url('dashboard/bidding-status-change?value=2',$biddingRequest->id)}}" class="btn btn-block btn-danger "
                         >Reject</a>
                        @endif
                ...
                <form method="POST" enctype="multipart/form-data" id="bidcForm" @if(isset($biddingRequest)) action="{{url('dashboard/accept-bidding-request', $biddingRequest->id)}} " @endif>
                  ...
                  <input type="text" class="status-change-bidding-id" hidden @if(isset($biddingRequest)) value="{{$biddingRequest->id}}" @endif>
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="hidden" name="coupon_code_id" class="coupon-code-id" id="">
            ...
            <div class="modal-content mdc">
                <form method="POST" enctype="multipart/form-data" id="bidForm">
                    ...
                    <div class="modal-body mdb bidModal">
                        ...
                        <p>
                            <label>Course</label>
                            <select name="course" id="" class="select-course form-control form-control-lg course_id">
                    ...
                    <input type="hidden" name="coupon-code-ids" class="coupon-code-ids" id="">
                    <div class="modal-footer">
                        {{-- <div class="col-md-4 px-2">
                            <button type="button" class="btn my-1 btn-danger btn-block accept-delete"
                                data-dismiss="modal">Reject</button>
...
@section('page-js')
    <script>
        ...
        $('.status-model').on('click', function() {
            biddingId = $(this).data('id');
            $('.coupon-code-id').val(biddingId);
            $('#bid-modal').modal('show');
        })

        $('.coupon-model').on('click', function() {
           var categoryId = $(this).data('content');
            couponId = $(this).data('id');
            $('.coupon-code-ids').val(couponId);
            $('#coupon-modal').modal('show');

            $.ajax({
                url:base_url + "/dashboard/category-base-course",
                type:"get",
                data:{category_id:categoryId},
                success:function(data)
                {
                    $('.select-course').html(data.data);
                },error:function(data)
                {
...
//         })
    ...
    <script>
        $('.coupon-code-btn').on('click', function() {
            //  alert('hai');
            var coupon_code_id = $('.coupon-code-ids').val();
            var title = $('.title').val();
