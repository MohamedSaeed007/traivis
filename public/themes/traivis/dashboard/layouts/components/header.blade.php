...
<head>
    ...
    {{-- Dark Mode --}}
    <link rel="stylesheet" href="{{ asset('assets/css/dark-mode-student.css') }}">
...
<div class="main-navbar-wrap">
    ...
    <nav class="navbar navbar-expand navbar-light topbar static-top imphead">
        ...
        <form class="d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
            <div class="input-group">
                <a class="navbar-brand site-main-logo mlm20" href="{{ route('home') }}">
                    ...
                    @if ($logoUrl)
                        <img loading="lazy" src="{{ media_file_uri(get_option('site_logo')) }}"
                             alt="{{ get_option('site_title') }}" class="img-responsive"/>
                    @else
                        <img loading="lazy" src="{{ asset('assets/images/logo.png') }}" alt="{{ get_option('site_title') }}"
                             width="30%" class="loxs"/>
                    @endif
                </a>
            </div>
        ...
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown no-arrow">
                <a class="nav-link dropdown-toggle mt5" href="#" id="userDropdown" role="button"
                   data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    @php
                        $checkUserAdminIsCompany = \App\Models\CompanyWorkforce::join('users', 'users.id', 'company_workforce.company_id')
                            ->where('company_workforce.user_id', Auth::user()->id)
                            ->first();
                    @endphp
            ...
            <li class="nav-item mt-1 ml-2 pt-1">
